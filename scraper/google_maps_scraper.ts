/**
 * google_maps_scraper.ts
 * 
 * Nasaka Supplementary Geocoding Scraper v14.0
 * 
 * Architecture:
 *  - Targets ALL rows in public.iebc_registration_centres
...
const OUTPUT_CSV = path.resolve(__dirname, '../data/iebc_google_supplementary.csv');
const CURSOR_FILE = path.resolve(__dirname, '../data/scraper_cursor.json');
const BATCH_SIZE = 50;
...
async function main() {
    console.log('[GEOC-v14] Initializing Google Maps Full-Scale Scraper...');
    console.log('[GEOC-v14] Target: ALL rows in public.iebc_registration_centres');
    ...
    const countResult = await pg.query('SELECT count(*) as total FROM public.iebc_registration_centres');
    ...
    while (true) {
        ...
        try {
            const result = await pg.query(`
                SELECT id::text as id, name, county, constituency, ward, centre_code
                FROM public.iebc_registration_centres
                ORDER BY county, constituency, name
                OFFSET $1 LIMIT $2
