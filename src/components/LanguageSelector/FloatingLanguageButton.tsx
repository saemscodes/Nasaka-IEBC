  ...
  useEffect(() => {
    ...
    if (!physicsStatesRef.current.has(stateId)) {
      physicsStatesRef.current.set(stateId, {
        x: position.x,
        y: position.y,
        velocityX: (Math.random() - 0.5) * 10, // Reduced velocity for smaller radius
  ...
  useEffect(() => {
    ...
    const animateFloat = (time: number) => {
      ...
      if (state) {
        ...
        // Calculate floating offset based on personality - smaller amplitude for 20px radius
        const floatAmplitude = personality.floatAmplitude * 0.3; // Reduced for smaller radius
        const floatX = Math.sin(currentPhaseRef.current + personality.floatPhase) * floatAmplitude;
        const floatY = Math.cos(currentPhaseRef.current * 0.7 + personality.floatPhase) * floatAmplitude;
