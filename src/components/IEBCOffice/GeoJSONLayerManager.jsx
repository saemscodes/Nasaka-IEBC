...
const GeoJSONLayerManager = ({ 
  ...
  const fetchLayerData = useCallback(async (layerId) => {
    ...
    try {
      ...
      setLayerErrors(prev => ({ ...prev, [layerId]: error.message }));
    } finally {
  ...
  const createPopupContent = (properties, geometryCoords = null) => {
    ...
    const popupStyles = `
      <style>
        .popup-container { min-width: 280px; padding: 12px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; color: #1f2937; }
        .dark .popup-container { color: #f9fafb; }
        ...
        .dark .popup-verified { background: #14532d; color: #bbf7d0; }
        ...
        .dark .popup-error { background: #7f1d1d; border-color: #f87171; color: #fca5a5; }
        .navigation-section { margin-top: 16px; }
        .navigation-title { font-size: 11px; font-weight: 600; color: #374151; margin-bottom: 8px; }
        .dark .navigation-title { color: #d1d5db; }
        .navigation-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
        .nav-button { display: flex; align-items: center; justify-content: center; gap: 4px; padding: 8px 12px; border: none; border-radius: 8px; font-size: 11px; font-weight: 500; color: white; cursor: pointer; transition: all 0.2s ease; text-decoration: none; }
        .nav-button:hover { opacity: 0.9; transform: translateY(-1px); }
        .copy-button { width: 100%; margin-top: 8px; padding: 8px 12px; border: none; border-radius: 8px; font-size: 11px; font-weight: 500; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 4px; transition: all 0.2s ease; }
        ...
        .nav-button .icon, .copy-button .icon { fill: #ffffff; }
      </style>
    `;
    ...
    return `${popupStyles}
      <div class="popup-container">
        ...
        <div class="popup-details">
          ...
          <div class="popup-detail">
            <svg class="icon" fill="currentColor" viewBox="0 0 24 24">
              <path d="M19 21V5a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v16m14 0h2m-20 0h-5m9-18H9m0 0H7m20 4v4m0 4h-10m-10-4h4m-4-4h6"/>
            </svg>
            ...
            <div class="navigation-grid">
              <button onclick="window.open('https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving', '_blank')" class="nav-button car" title="Navigate by car">
                <svg class="icon" viewBox="0 0 24 24">
                  <path d="M19.78 9.44L17.94 9.44C17.82 9.096 17.603 9.796 17.309 9.583C17.015 9.37 16.663 9.253 16.33 9.25H7.7C7.341 9.250 6.992 9.361 6.699 9.567C6.405 9.772 6.182 10.063 6.06 10.4L4.22 10.4C3.924 9.541 3.674 9.762 3.498 10.037C3.321 10.313 3.225 10.632 3.22 10.96V15.46C3.214 15.752 3.282 16.041 3.419 16.3C3.555 16.559 3.755 16.779 4 16.94V17V19C4 19.265 4.105 19.519 4.292 19.707C4.48 19.894 4.734 20 5 20H6C6.265 20 6.519 19.894 6.707 19.707C6.894 19.519 7 19.265 7 19V17.25H17V19C17 19.265 17.105 19.519 17.292 19.707C17.48 19.894 17.734 20 18 20H19C19.265 20 19.519 19.894 19.707 19.707C19.894 19.519 20 19.265 20 19V17C20 17 20 17 20 16.94C20.235 16.78 20.427 16.566 20.561 16.315C20.692 16.063 20.761 15.784 20.761 15.5V11C20.756 10.674 20.663 10.356 20.49 10.081C20.317 9.806 20.071 9.584 19.78 9.44ZM19.25 15.5C19.25 15.566 19.223 15.629 19.176 15.676C19.129 15.723 19.066 15.75 19 15.75H5C4.933 15.75 4.870 15.723 4.823 15.676C4.776 15.629 4.75 15.566 4.75 15.5V11C4.75 10.933 4.776 10.870 4.823 10.823C4.870 10.776 4.933 10.75 5 10.75H19C19.066 10.75 19.129 10.776 19.176 10.823C19.223 10.870 19.25 10.933 19.25 11V15.5ZM7.47 10.91C7.487 10.863 7.519 10.823 7.560 10.794C7.601 10.766 7.650 10.750 7.7 10.75H16.3C16.349 10.750 16.398 10.766 16.439 10.794C16.480 10.823 16.512 10.863 16.53 10.91L17.93 10.75H6.07L7.47 10.91Z"/>
              ...
              <button onclick="window.open('https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=transit', '_blank')" class="nav-button transit" title="Navigate via public transport">
                <svg class="icon" viewBox="0 0 64 64">
                  <path d="M52,0H12C5.375,0,0,5.371,0,12v40c0,2.211,1.789,4,4,4h4v4c0,2.211,1.789,4,4,4h4c2.211,0,4-1.789,4-4v-4h24v4c0,2.211,1.789,4,4,4h4c2.211,0,4-1.789,4-4v-4h4c2.211,0,4-1.789,4-4V12C64,5.375,58.629,0,52,0zM16,44c-2.211,0-4-1.789-4-4s1.789-4,4-4s4,1.789,4,4S18.211,44,16,44zM48,44c-2.211,0-4-1.789-4-4s1.789-4,4-4s4,1.789,4,4S50.211,44,48,44zM56,24H8V12c0-2.211,1.789-4,4-4h40c2.211,0,4,1.789,4,4V24z"/>
                </svg>
                <span>Transit</span>
              </button>
        ...
        ` : `
          <div class="popup-error">Location coordinates are not available for this office.</div>
        `}
      </div>
    `;
  };
  ...
  useEffect(() => {
    ...
    if (activeLayers.includes('iebc-offices') && liveOfficesGeoJSON.features.length > 0) {
      setLayerData(prev => ({ ...prev, 'iebc-offices': liveOfficesGeoJSON }));
    }
