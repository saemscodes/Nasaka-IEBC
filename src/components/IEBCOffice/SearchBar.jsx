...
const SearchBar = ({
  ...
  // Fix 3: Navigate to office detail page
  const handleViewDetails = useCallback((suggestion) => {
    if (!suggestion.county || !suggestion.constituency_name) return;
  ...
  const highlightMatches = (text, matches) => {
    ...
    matchIndices.forEach(([start, end]) => {
      ...
      result.push(
        <span key={start} className={`px-1 rounded font-semibold transition-all duration-300 ${theme === 'dark'
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className={`search-container transition-all duration-300 ${theme === 'dark'
        ...
        <div className="flex items-center space-x-3">
          <div className="pl-4">
            <Search className={`w-5 h-5 transition-colors duration-300 ${theme === 'dark' ? 'text-ios-dark-text-secondary' : 'text-ios-light-text-secondary'
              }`} />
          </div>
          ...
          <div className="flex-1 relative min-w-0">
            <input
              ...
              className={`w-full bg-transparent border-none outline-none placeholder:text-muted-foreground text-base py-2 px-1 pr-16 transition-colors duration-300 ${theme === 'dark'
            ...
            <div className="absolute inset-y-0 right-2 flex items-center space-x-2">
              {value && (
                <motion.button
                  ...
                  onClick={handleClear}
                  className={`flex items-center justify-center p-1 rounded-full transition-all duration-200 ${theme === 'dark'
        ...
        <AnimatePresence>
          {isExpanded && (suggestions.length > 0 || isLoading) && (
            <motion.div
              className={`absolute top-full left-0 right-0 mt-2 border rounded-2xl shadow-2xl overflow-hidden max-h-96 overflow-y-auto transition-all duration-300 ${theme === 'dark'
            ...
            >
              {isLoading ? (
                ...
                <>
                      ...
                      >
                        <div className="flex">
                          <button
                            ...
                            className={`flex-1 text-left p-4 transition-all duration-200 ${theme === 'dark'
                          ...
                          >
                            <div className="flex items-start space-x-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className={`font-semibold text-base truncate transition-colors duration-300 ${theme === 'dark' ? 'text-ios-dark-text-primary' : 'text-ios-light-text-primary'
                                  }`}>
                                  {display.primary}
                                </div>
                                <div className={`text-sm truncate mt-1 transition-colors duration-300 ${theme === 'dark' ? 'text-ios-dark-text-secondary' : 'text-ios-light-text-secondary'
                                  }`}>
                                  {display.secondary}
                                </div>
                                {display.tertiary && (
                                  <div className={`text-xs truncate mt-2 transition-colors duration-300 ${theme === 'dark' ? 'text-ios-dark-text-tertiary' : 'text-ios-light-text-tertiary'
                                    }`}>
                                    {display.tertiary}
                                  </div>
                                )}
                          ...
                          {/* Fix 3: Detail page link for office suggestions */}
                          {suggestion.type === 'office' && suggestion.county && suggestion.constituency_name && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleViewDetails(suggestion);
                              }}
                              className={`flex items-center px-3 border-l transition-all duration-200 ${theme === 'dark'
                                ? 'border-ios-dark-border hover:bg-ios-dark-surface-hover text-ios-blue-light'
                                : 'border-ios-light-border hover:bg-ios-light-surface-hover text-ios-blue-dark'
                                }`}
                  ...
                  <div className={`p-4 border-t transition-all duration-300 ${theme === 'dark'
