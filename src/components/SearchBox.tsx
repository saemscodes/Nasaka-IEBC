...
const AnimatedSearchIcon = ({ 
  ...
  const resetToSearchAndPauseCycle = () => {
    clearTimers();
    setCurrentIcon(0);
    setIsAnimating(false);
    lastInteractionRef.current = Date.now();

    // Restart cycling after 10 seconds of inactivity
    resumeTimeoutRef.current = setTimeout(() => {
      if (isMountedRef.current && isIdle && !isHovered && !isActive && !isLoading) {
        startCycle();
      }
    }, 10000);
  };
  ...
  const startCycle = () => {
    clearTimers();
    let lastTime = 0;
    const cycleDuration = 2000; // 2 seconds per icon

    const animate = (time: number) => {
      if (!isMountedRef.current) return;

      // Skip if not idle or if recently interacted
      if (!isIdle || isHovered || isActive || isLoading || 
  ...
  // Handle interactions from parent component
  useEffect(() => {
    resetToSearchAndPauseCycle();
  }, [interactionSignal]);
  ...
  }, [isIdle, isHovered, isActive, isLoading]);
