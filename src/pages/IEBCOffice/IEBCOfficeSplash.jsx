...
const CekaLogoButton = () => {
  ...
  const handleCekaClick = () => {
    window.open('https://www.civiceducationkenya.com/join-community', '_blank', 'noopener,noreferrer');
  };
...
const IEBCOfficeSplash = () => {
  ...
  return (
    <div className={`relative flex flex-col min-h-screen transition-colors duration-500 overflow-hidden ${theme === 'dark'
      ...
      {/* SEO Head — structured data for the IEBC finder landing page */}
      <SEOHead
        title={t('seo.splashTitle', 'Find Your Nearest IEBC Office — Nasaka IEBC')}
        description={t('seo.splashDescription', 'Locate the nearest IEBC constituency or voter registration office in Kenya. Get directions, opening hours, and community-verified location data.')}
        canonical="/iebc-office"
        keywords={t('seo.keywords', 'IEBC Kenya, IEBC office near me, voter registration Kenya, CVR Kenya, IEBC constituency offices, Nasaka IEBC, find IEBC office, nearest IEBC office')}
        schema={[
          generateWebsiteSchema(),
          generateFAQSchema([
            { question: 'How do I find my nearest IEBC office?', answer: 'Use the Nasaka IEBC interactive map to locate any of the 290+ IEBC constituency offices across Kenya. Allow location access for the closest office, or search manually by county or constituency name.' },
            { question: 'What do I need to register as a voter in Kenya?', answer: 'Bring your Kenyan National ID Card or valid Passport to any IEBC constituency office during a Continuous Voter Registration (CVR) period. The process takes about 5-10 minutes.' },
            { question: 'When are IEBC offices open?', answer: 'IEBC constituency offices typically operate Monday to Friday, 8:00 AM to 5:00 PM EAT. Hours may vary during voter registration drives.' },
            { question: 'Can I transfer my voter registration?', answer: 'Yes. Visit the IEBC office in your desired constituency with your national ID during any open CVR period to transfer your voter registration.' },
          ]),
        ]}
      />
      ...
      >
        ...
        <div className="relative z-10 text-center max-w-md w-full mb-8">
          ...
          <TextShadowLayer className="mb-8">
            ...
            >
              {t('splash.disclaimer', 'Allow location access to find the closest IEBC registration center and get turn-by-turn navigation.')}
            </motion.p>
          </TextShadowLayer>
          ...
          <TextShadowLayer className="mt-8">
            <motion.p
              className={`text-xs ${theme === 'dark' ? 'text-ios-gray-400' : 'text-ios-gray-500'
                }`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
